...
def upload_image():
    ...
    file_open = make_grayscale(file.read())
...
def make_grayscale(input_image):
    ...
    decode2img = cv2.imdecode(image_array , cv2.IMREAD_UNCHANGED)
    print('Decode2image:-', decode2img)

    decode2grayscale = cv2.cvtColor(decode2img, cv2.COLOR_RGB2GRAY)
    print('Grayscale image:-', decode2grayscale)
    status, encoded_image = cv2.imencode('.PNG', decode2grayscale)
    print('Status:- ', status)
